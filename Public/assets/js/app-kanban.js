...
(async function () {
  ...
  if (!kanbanResponse.ok) {
    console.error('error fetching kanban data', kanbanResponse);
  }
...
`;
  ...
  const kanban = new jKanban({
    ...
    dropEl: (el, target) => {
      ...
      const boardEl = target ? target.closest('.kanban-board') : null;
      if (!itemId || !boardEl) return;
      fetch(`/api/kanban/items/${itemId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ board_id: boardEl.getAttribute('data-id') })
      }).catch(err => console.error('Failed to persist drag:', err));
    },
    ...
    buttonClick: (el, boardId) => {
      ...
      addNewForm.addEventListener('submit', async e => {
        ...
        const itemId = `${boardId}-${Date.now()}`;
        kanban.addElement(boardId, {
          title: `<span class="kanban-text">${text}</span>`,
          id: itemId
        });
        fetch(`/api/kanban/boards/${boardId}/items`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: itemId, title: text })
        ...
        // Wire delete for newly added tasks
        wireDeleteTasks();
  ...
  if (kanbanItem.length) {
    kanbanItem.forEach(el => {
      ...
      if (el.getAttribute('data-badge') && el.getAttribute('data-badge-text')) {
        ...
        // Plain item from DB — insert dropdown + text with no badge
        el.insertAdjacentHTML('afterbegin', `${renderDropdown()}${element}`);
      }
  ...
  deleteBoards.forEach(elem => {
    elem.addEventListener('click', () => {
      ...
      fetch(`/api/kanban/boards/${id}`, { method: 'DELETE' })
        .catch(err => console.error('Failed to delete board:', err));
      kanban.removeBoard(id);
    });
  });

  // Delete task for rendered boards — runs after render loop so dropdowns exist
  function wireDeleteTasks() {
    document.querySelectorAll('.delete-task').forEach(task => {
      if (task.dataset.wired) return;
      task.dataset.wired = 'true';
      task.addEventListener('click', () => {
        const id = task.closest('.kanban-item').getAttribute('data-eid');
        fetch(`/api/kanban/items/${id}`, { method: 'DELETE' })
  ...
  if (kanbanAddNewBoard) {
    kanbanAddNewBoard.addEventListener('submit', e => {
      ...
      const id = value.replace(/\s+/g, '-').toLowerCase() + '-' + Date.now();
